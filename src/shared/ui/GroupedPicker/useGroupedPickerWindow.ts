...
import {
  findGroupedPickerRowIndexAtOffset,
  getGroupedPickerVirtualRange,
  type GroupedPickerRowsModel
...
export function useGroupedPickerWindow(args: {
  ...
  const updateViewport = useCallback(() => {
    ...
    const rowIndex = findGroupedPickerRowIndexAtOffset(rowsModel.rows, scroller.scrollTop + 12);
  ...
  const revealItem = useCallback((itemId: string, align: 'nearest' | 'start' = 'nearest') => {
    const scroller = scrollerRef.current;
    const rowIndex = rowsModel.itemRowIndexes.get(itemId);
    ...
    updateViewport();
    return true;
  }, [rowsModel.itemRowIndexes, rowsModel.rows, updateViewport]);

  const resetScroll = useCallback((itemId?: string) => {
    const scroller = scrollerRef.current;
    if (!scroller) return;
    const rowIndex = itemId ? rowsModel.itemRowIndexes.get(itemId) : undefined;
    ...
    lastScrollRef.current = { top: scroller.scrollTop, time: 0 };
    updateViewport();
  }, [rowsModel.groupOffsets, rowsModel.itemRowIndexes, rowsModel.rows, updateViewport]);

  const visibleItemCount = visibleRows.reduce((count, row) => count + (row.kind === 'item' ? 1 : 0), 0);
