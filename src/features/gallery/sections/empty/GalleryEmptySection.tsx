  ...
  return (
    <div className={styles.empty} data-gallery-slot="empty">
      ...
      {filteredEmpty ? (
        ...
        <div className={styles.emptyActions} aria-label={t('gallery.emptyActions')}>
          <Button variant="primary" size="compact" onClick={focusPrompt}>{t('gallery.emptyCreate')}</Button>
          <span>{t('gallery.emptySteps')}</span>
        </div>
      )}
    </div>
  );
